<?php
	...
	# last 28-jun-2011
	# ---------------------------------------------
	# read the Solar H-alpha Flare events
	# http://www.ngdc.noaa.gov/stp/SOLAR/ftpsolarflares.html#halpha
	# format like
...
/*
		...
		exec ("/bin/rm temp/goes/XRAY*");
		exec ("wget -P temp/goes -N -A 'XRAY[1-9]*','xray[1-9]*' ftp://anonymous@ftp.ngdc.noaa.gov/STP/SOLAR_DATA/SOLAR_FLARES/XRAY_FLARES/\*");
		exec ("cat temp/goes/xray* temp/goes/XRAY* > all_xray.txt");
		copy ("all_xray.txt",$tempdir."/all_xray.txt");
		unlink ("all_xray.txt");
*/
                exec("wget -nc -P temp/HALPHA_FLARES ftp://ftp.ngdc.noaa.gov/STP/SOLAR_DATA/SOLAR_FLARES/HALPHA_FLARES/f_report*");
                exec("/bin/rm temp/HALPHA_FLARES/f_report.fmt.revised");
                exec("cat temp/HALPHA_FLARES/* > ".$tempdir."/all_flares.txt");
	}
	
	// parse files and create postgres-ready file	
	$f1 = fopen("$tempdir/HA.postgres.converted",'w');
//wget  wget ftp://ftp.ngdc.noaa.gov/STP/SOLAR_DATA/SOLAR_FLARES/HALPHA_FLARES/f_report* in temp/flares
//cat temp/flares/f_report* > temp/all_flares.txt
//still errors in parsed HA.postgres.converted, edit manually!!!
	$f2 = fopen("$tempdir/all_flares.txt",'r');
	$ntime_err=0;
...
//			if ($t1h>23) echo "23>\n";
        ...
        //dirty minutes check       
        //echo( ">".substr($buf[3],14,2)."<\n\n"); 
        if (substr($buf[1],14,2)>59) $buf[1]="";
        if (substr($buf[2],14,2)>59) $buf[2]="";
        if (substr($buf[3],14,2)>59) $buf[3]="";
        if (!is_numeric(substr($buf[1],14,2))) $buf[1]="";
        if (!is_numeric(substr($buf[2],14,2))) $buf[2]="";
        if (!is_numeric(substr($buf[3],14,2))) $buf[3]="";
